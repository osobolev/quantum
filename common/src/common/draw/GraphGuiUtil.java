...
public final class GraphGuiUtil {
    ...
    public final AbstractAction pauseAction = new AbstractAction("Pause") {
        public void actionPerformed(ActionEvent e) {
            pauseModel();
        }
    };
    public final AbstractAction optionsAction = new AbstractAction("Options") {
        public void actionPerformed(ActionEvent e) {
    ...
    private void doRun(boolean showPhotons) {
        try {
            running = new Running(panel, options, showPhotons, factory);
    ...
    static void setTitle(JFrame frame, GraphPanel panel, String title) {
        int revision = panel.getRevision();
        String rtitle = VersionUtil.getTitle(revision, title);
        frame.setTitle(rtitle + (panel.getSaved() == null ? "" : " - " + panel.getSaved().getName()));
    }
